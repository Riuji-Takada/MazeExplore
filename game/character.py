...
class Character:
    ...
    async def can_move_forward(self):
        ...
        has_wall = position_rect.collidelist(self.__wall_list) == -1
        return has_wall
    ...
    def draw(self, screen):
        ...
        # キャラクターの移動処理（座標変更）
        if self.__can_move():
            self.__position = self.__position.move_towards(
                self.__next_position, self.__speed)
        ...
        # 画像サイズをタイルサイズに合わせてスケールする
        scaled_image = pygame.transform.scale(
            animation[index], (constants.TILE_SIZE, constants.TILE_SIZE))
        # キャラクターの描画処理
