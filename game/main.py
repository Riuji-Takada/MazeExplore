...
class MazeGame:
    def __init__(self, map_data, configuration_data):
        ...
        character_start_pos = get_configuration_position(
            configuration_data, Configuration.CHARACTER_START_POSITION)
        # キャラクターの初期化
        self.__character = Character(
            character_start_pos, self.__map.get_wall_list())

        # マップ配置データからゴールの初期座標を取得
        goal_position = get_configuration_position(
            configuration_data, Configuration.GOAL)
        # ゴールの初期化
        ...
        # ゲームクリアメッセージ画像の生成
        self.__game_clear_text = game_font.render(
            "Game Clear!", False, (230, 255, 148), (0, 103, 105))

        # ゲームオーバーメッセージ
        self.__game_over_text = game_font.render(
            "Game Over!", False, (243, 254, 184), (255, 76, 76))
    ...
    def draw_text_centered(
        ...
        if draw_border:
            ...
            border_rect = pygame.Rect(
                text_rect.x - border_size,
                text_rect.y - border_size,
                text_rect.width + border_size * 2,
                text_rect.height + border_size * 2)
...
def get_configuration_position(
    ...
    for i in range(max_i):
        ...
        for j in range(max_j):
            if configuration_data[j][i] == config_type.value:
